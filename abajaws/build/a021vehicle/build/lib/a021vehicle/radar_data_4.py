...
# from std_msgs.msg import Int32 #removed as no phase_id
...
class RadarFilterNode(Node):
    def __init__(self):
        ...
        # Timers: One for clean up, one for continuous phase publishing
        self.create_timer(0.1, self.purge_stale_targets)
        # self.phase_timer = self.create_timer(0.1, self.publish_phase_continuous)

        self.get_logger().info("Radar Filter Node started. Continuous phase publishing active.")
    ...
    # def publish_phase_continuous(self):
    #     """Timer callback that continuously publishes phase status at 10Hz"""
    #     phase_msg = Int32()
    ...
    def raw_radar_callback(self, msg: RadarMessage):
        ...
        # Always log valid target details
        ...
        # ── Latching Alert Logic (Updates state variable only) ───────────
        if x_m < 10.0:
            if not self.alert_latched:
